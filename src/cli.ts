...
async function api(method: string, path: string, body?: any): Promise<any> {
  ...
  if (!res.ok) die(data.error || `API error (${res.status})`);
  return data;
}
...
function ok(msg: string) { if (!JSON_OUT) console.log(`  ${msg}`); }
...
async function cmdAuth(args: string[]) {
  const key = args[0];
  if (!key) die('Usage: vibekit auth <api-key>\nGet yours at https://app.vibekit.bot/settings');
  if (!key.startsWith('vk_')) die('Invalid key format — keys start with vk_');
...
async function cmdApps() {
  ...
  const apps = d.apps || d;
  if (JSON_OUT) return out(apps);
  if (!apps.length) return ok('No apps yet. Create one at https://app.vibekit.bot');
  table([
...
async function cmdAppInfo(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit app <id>');
  ...
  if (d.customDomain) console.log(`  Domain:     ${d.customDomain}`);
  if (d.addons?.database) console.log(`  Database:   provisioned`);
}

// ── Deploy ───────────────────────────────────────────────────────────────────

async function cmdDeploy(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit deploy <id>');
  const d = await api('POST', `/hosting/app/${id}/redeploy`);
  if (JSON_OUT) return out(d);
  ok(`Deploy triggered for ${id}`);
  if (d.message) ok(d.message);
}

async function cmdDeploys(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit deploys <id>');
  const d = await api('GET', `/hosting/app/${id}/deploys`);
  if (JSON_OUT) return out(d);
  const deploys = d.deploys || d;
  if (!deploys.length) return ok('No deploys yet');
  table([
    ['ID', 'Status', 'Trigger', 'When'],
    ...deploys.slice(0, 20).map((dep: any) => [
...
async function cmdRollback(args: string[]) {
  ...
  if (!id || !deployId) die('Usage: vibekit rollback <app-id> <deploy-id>');
...
async function cmdLogs(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit logs <id>');
  ...
  else if (Array.isArray(logs)) logs.forEach((l: any) => console.log(typeof l === 'string' ? l : `${l.timestamp || ''} ${l.message || l.text || JSON.stringify(l)}`));
  else out(logs);
}
...
async function cmdChat(args: string[]) {
  ...
  if (!id || !msg) die('Usage: vibekit chat <app-id> "your message"');
  const d = await api('POST', `/hosting/app/${id}/agent`, { message: msg });
  if (JSON_OUT) return out(d);
  if (d.response) console.log(d.response);
  else if (d.message) console.log(d.message);
  else ok('Message sent to agent');
}

async function cmdAgentStatus(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit agent <id>');
  ...
  if (d.roles) console.log(`  Roles:      ${d.roles.join(', ')}`);
}

// ── Env Vars ─────────────────────────────────────────────────────────────────

async function cmdEnv(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit env <id> [set KEY=VAL | del KEY]');
  const action = args[1];

  if (action === 'set') {
    const pair = args[2]; if (!pair || !pair.includes('=')) die('Usage: vibekit env <id> set KEY=VALUE');
    const [key, ...rest] = pair.split('=');
    const value = rest.join('=');
    await api('POST', `/hosting/app/${id}/env`, { vars: { [key]: value } });
    ok(`Set ${key}`);
  } else if (action === 'del' || action === 'delete') {
    const key = args[2]; if (!key) die('Usage: vibekit env <id> del KEY');
...
async function cmdStart(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit start <id>');
  await api('POST', `/hosting/app/${id}/start`);
  ok('App started');
}

async function cmdStop(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit stop <id>');
  await api('POST', `/hosting/app/${id}/stop`);
  ok('App stopped');
}

async function cmdRestart(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit restart <id>');
...
async function cmdDb(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit db <id> [query "SQL" | schema | tables <name> | export]');
  const action = args[1] || 'status';

  if (action === 'query') {
    const sql = args.slice(2).join(' ');
    if (!sql) die('Usage: vibekit db <id> query "SELECT * FROM users"');
    const d = await api('POST', `/hosting/app/${id}/database/query`, { sql });
    if (JSON_OUT) return out(d);
    console.log(`  ${d.rowCount} rows (${d.durationMs}ms)\n`);
    if (d.rows?.length) {
      table([d.columns, ...d.rows.map((r: any) => d.columns.map((c: string) => truncate(String(r[c] ?? 'null'), 40)))]);
    }
    ...
    const tableName = args[2]; if (!tableName) die('Usage: vibekit db <id> tables <table-name>');
    ...
    if (d.rows?.length) {
      table([d.columns, ...d.rows.map((r: any) => d.columns.map((c: string) => truncate(String(r[c] ?? 'null'), 30)))]);
    }
  } else if (action === 'export') {
    const res = await apiRaw('GET', `/hosting/app/${id}/database/export`);
    if (!res.ok) die('Export failed');
    ...
    if (JSON_OUT) return out(d);
    if (!d.provisioned) return ok('No database provisioned');
...
async function cmdFiles(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit files <id> [path]');
  const path = args[1] || '';
...
async function cmdQa(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit qa <id>');
  const d = await api('POST', `/hosting/app/${id}/qa`);
  if (JSON_OUT) return out(d);
  ok('QA run started');
  if (d.runId) ok(`Run ID: ${d.runId}`);
}
...
async function cmdCollaborators(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit collaborators <id> [add <email> | remove <id>]');
  const action = args[1];

  if (action === 'add') {
    const email = args[2]; if (!email) die('Usage: vibekit collaborators <id> add <email>');
    ...
    const collabId = args[2]; if (!collabId) die('Usage: vibekit collaborators <id> remove <collab-id>');
    ...
    if (!collabs?.length) return ok('No collaborators');
    table([
      ['ID', 'Email', 'Role', 'Status'],
      ...collabs.map((c: any) => [c.id?.slice(0, 8) || '-', c.email || '-', c.role || '-', c.status || '-']),
    ]);
  }
}
...
async function cmdTask(args: string[]) {
  ...
  if (!prompt) die('Usage: vibekit task "Build a landing page"');
  const repo = getFlag(args, '--repo');
  const branch = getFlag(args, '--branch');
  const body: any = { prompt };
  if (repo) body.repo = repo;
  if (branch) body.branch = branch;
  if (args.includes('--no-deploy')) body.deploy = false;
...
async function cmdTaskStatus(args: string[]) {
  const taskId = args[0]; if (!taskId) die('Usage: vibekit status <task-id>');
  const d = await api('GET', `/task/${taskId}`);
  if (JSON_OUT) return out(d);
  console.log(`  Task:    ${d.taskId}`);
  console.log(`  Status:  ${d.status}`);
  if (d.result?.deployUrl) console.log(`  URL:     ${d.result.deployUrl}`);
...
async function cmdDomain(args: string[]) {
  const id = args[0]; if (!id) die('Usage: vibekit domain <id> [set <domain> | ssl]');
  const action = args[1];
  if (action === 'set') {
    const domain = args[2]; if (!domain) die('Usage: vibekit domain <id> set <domain>');
    const d = await api('POST', `/hosting/app/${id}/domain`, { domain });
    if (JSON_OUT) return out(d);
    ok(`Domain set to ${domain}`);
    if (d.cname) ok(`CNAME: ${d.cname}`);
  } else if (action === 'ssl') {
    ...
    const d = await api('GET', `/hosting/app/${id}`);
    if (JSON_OUT) return out({ domain: d.customDomain, subdomain: d.subdomain });
